...
def _gather(base_url, token, canvas_course_id, client):
    sources = [
        ("syllabus",
         lambda: [d for d in [fetch_syllabus(base_url, token, canvas_course_id, client)] if d]),
        ("pages",
         lambda: fetch_pages(base_url, token, canvas_course_id, client)),
        ("modules",
         lambda: fetch_module_items(base_url, token, canvas_course_id, client)),
        ("assignments",
         lambda: _assignment_documents(base_url, token, canvas_course_id, client)),
        ("announcements",
         lambda: fetch_announcements(base_url, token, canvas_course_id, client)),
        ("pdfs",
         lambda: fetch_pdf_documents(base_url, token, canvas_course_id, client)),
    ]
    docs = []
